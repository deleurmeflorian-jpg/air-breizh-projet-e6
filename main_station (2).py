...
GPIO.output(GPIO_VENTILO, GPIO.LOW)   # ventilateur OFF au démarrage
GPIO.output(GPIO_VERTE,   GPIO.LOW)
GPIO.output(GPIO_ROUGE,   GPIO.LOW)
GPIO.output(GPIO_RELAIS,  GPIO.LOW)

log.info("GPIO initialisé — VENTILO:GPIO%d VERTE:GPIO%d ROUGE:GPIO%d RELAIS:GPIO%d ILS:GPIO%d",
...
def thread_ils():
    ...
    while not stop.is_set():
        ...
        if ouvert and t_ouverture and (time.time() - t_ouverture) > 300:
            log.warning("ALERTE — porte toujours ouverte depuis %d s !", int(time.time() - t_ouverture))
...
def thread_dht20():
    """
    Lit la température et l'humidité via le DHT20 (AHTx0 sur I2C).

    Règle ventilateur et relais :
        température > 26 °C  →  ventilateur ON  + relais ON
        température ≤ 26 °C  →  ventilateur OFF + relais OFF
    """
    global _alerte_temp
    try:
        import board
        ...
        while not stop.is_set():
            ...
            # ── Logique ventilateur ────────────────────────────────────────
            # Le ventilateur se déclenche UNIQUEMENT quand la température
            # dépasse le seuil de 26 °C, et s'arrête dès qu'elle redescend.
            if temperature > SEUIL_TEMP:
                GPIO.output(GPIO_VENTILO, GPIO.HIGH)   # ventilateur ON
                GPIO.output(GPIO_RELAIS,  GPIO.HIGH)   # relais ON
                ventilo_on = True
                relais_on  = True
            else:
                GPIO.output(GPIO_VENTILO, GPIO.LOW)    # ventilateur OFF
                GPIO.output(GPIO_RELAIS,  GPIO.LOW)    # relais OFF
                ventilo_on = False
                relais_on  = False
            ...
            if ventilo_on and (time.time() - _alerte_temp) > 60:
                log.warning("ALERTE — température %.1f °C dépasse %.1f °C"
                            " (ventilateur + relais activés)", temperature, SEUIL_TEMP)
        ...
        # Sécurité : tout éteindre si le capteur plante
                ...
                try:
                    ...
                    if result is None:
                        ...
                        if pm25 > SEUIL_PM25 and (time.time() - _alerte_air) > 60:
                            log.warning("ALERTE — PM2.5 %.1f µg/m³ dépasse %.1f µg/m³ !",
                                        pm25, SEUIL_PM25)
        ...
        if t is not None and pm25 is not None:
            if (time.time() - _bdd_fail) > 60:
                try:
                    ...
                    cur.close()
                    conn.close()
                    log.info("BDD — enregistré | %.1f °C | %.1f %% | PM2.5:%.1f | PM10:%.1f"
                    ...
                    log.warning("BDD — erreur : %s (nouvelle tentative dans 60 s)", e)
                    _bdd_fail = time.time()
        else:
            log.debug("BDD — en attente de données capteurs...")
