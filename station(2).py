...
def thread_ils():
    """
    Affiche en PERMANENCE dans le terminal l'état de la porte toutes les 2 s.
    Affiche également un message spécial à chaque changement d'état
    (ouverture / fermeture) et une alerte si la porte reste ouverte > 5 min.
    """
    ...
    while not stop.is_set():
        ...
        # ── Affichage PERMANENT toutes les 2 s ────────────────────────────
        ...
        if precedent is not None and ouvert != precedent:
            if ouvert:
                ...
                if t_ouverture is not None:
                    ...
                    log.info("ILS — >>> PORTE VIENT DE SE FERMER (était ouverte %d s) <<<", duree)
                    if duree > 300:
                        log.warning("ILS — ALERTE : porte était ouverte plus de 5 min !")
                t_ouverture = None
